    ...
    public class UserImpl : IUser
    {
        ...
        public User Get(byte id)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    t = new User(dr[0].ToString(), dr[1].ToString(), dr[2].ToString(), byte.Parse(dr[3].ToString()), dr[4].ToString(), dr[5].ToString(), dr[6].ToString(), dr[7].ToString(),byte.Parse(dr[8].ToString()), short.Parse(dr[9].ToString()), DateTime.Parse(dr[10].ToString()), DateTime.Parse(dr[11].ToString()), byte.Parse(dr[12].ToString()));
                }
            }
            catch (Exception ex)
            {
        ...
        public int Insert(User t)
        {
            ...
            string query = @"INSERT INTO [User](role,UserFirstName,UserLastName,UserCI,UserEmail,userName,password,lastUpdate,registerUser)
                             VALUES(@role,@UserFirstName,@UserLastName,@UserCI,@UserEmail,@username,
                                    HASHBYTES('md5',@password),CURRENT_TIMESTAMP,@registerUser)";
            try
            {
                SqlCommand command = DataBase.CreateBasicCommand(query);
        ...
        public User GetAdmin(byte id)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    t = new User(byte.Parse(dr[0].ToString()), dr[1].ToString(), dr[2].ToString(),dr[3].ToString(), dr[4].ToString(), dr[5].ToString(), dr[6].ToString(), dr[7].ToString(), byte.Parse(dr[8].ToString()), short.Parse(dr[9].ToString()), DateTime.Parse(dr[10].ToString()), DateTime.Parse(dr[11].ToString()), byte.Parse(dr[12].ToString()));
                }
            }
            catch (Exception ex)
            {
